function renderInspector() {
  ...
  const bodyLabel={asset:'Data',instruction:'Instruction',expression:'Logic',check:'Check'}[n.kind] || 'Content';
...
function showRunStrip() {
  ...
  const ok=state.run.steps.filter(s=>s.status==='ok').length; box.innerHTML=`<span>${state.run.status==='ok'?'DONE':'STOPPED'}</span><strong>${ok}/${state.nodes.length}</strong><button aria-label="Close">×</button>`;
