...
const seedNodes = [
  ...
  { id:'instruction-1', kind:'instruction', title:'Frame', operation:'DETERMINISTIC', x:420, y:190, inputs:[{id:'in',name:'input',type:'text'}], outputs:[{id:'out',name:'text',type:'text'}], body:'State the claim:' },
...
function makeNode(kind) {
  ...
  if (kind==='instruction') return {id,kind,title:'Step',operation:'DETERMINISTIC',x,y,inputs:[{id:'in',name:'input',type:'text'}],outputs:[{id:'out',name:'text',type:'text'}],body:'Transform:'};
...
function renderNodes() {
  ...
  nodesEl.innerHTML = state.nodes.map(n=>{
    ...
    return `<div class="frame frame-${n.kind}${sel}${runClass}" data-frame="${n.id}" style="left:${n.x}px;top:${n.y}px;width:${W}px;height:${H}px">
      ...
      <div class="frame-meta"><span>${n.operation==='MODEL'?'MODEL':'LOCAL'}</span><span>${step?step.durationMs+'ms':(n.outputs[0]?.type||'sink')}</span></div>${ins}${outs}</div>`;
  }).join('');
}
